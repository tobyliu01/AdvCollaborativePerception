...
def compute_relative_bboxes(scenario_meta: Dict, frame_ids: List[int], object_id: int, ego_vehicle_id: int) -> Optional[np.ndarray]:
    rel_bboxes = []
...
def generate_entries(cases, dataset_meta: Dict, dataset_split: str):
    ...
    for case in cases:
        ...
        for ego_vehicle_id in vehicle_ids:
            rel_bboxes = compute_relative_bboxes(scenario_meta, frame_ids, object_id, ego_vehicle_id)
