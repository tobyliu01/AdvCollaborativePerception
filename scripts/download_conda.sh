...
if [ ! -f models/model_experiment.zip ]; then
	...
	conda --live-stream run -n $env_name gdown -O models/model_experiment.zip 1exCVzj7I7f201MOvCHFldBMsjA9oBF6D
...
if [ ! -f data/data_experiment.zip ]; then
	...
	conda --live-stream run -n $env_name gdown -O data/data_experiment.zip 1KW1Ya8CGvJuqB5dUBmH968zDFbBFgNtm
...
if [ ! -f data/OPV2V/test.zip ]; then
	...
	conda --live-stream run -n $env_name gdown -O data/OPV2V/test.zip 11JdX113eVs6wtCS0c5j7L6KfQcJQn2h0
