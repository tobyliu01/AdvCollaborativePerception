import os
import numpy as np

mvp_root = os.path.join(os.path.abspath(os.path.dirname(__file__)), "../")
external_storage_root = "/workspace/hdd/datasets/yutongl/AdvCollaborativePerception"
data_root = os.path.join(external_storage_root, "data")
model_root = os.path.join(external_storage_root, "models")
third_party_root = os.path.join(mvp_root, "third_party")
tmp_root = os.path.join(mvp_root, "tmp")
model_3d_path = os.path.join(data_root, "model_3d")
patch_path = os.path.join(data_root, "patch")

model_3d_examples = {
    "car_000000": np.array([0,0,0,5.00,2.00,1.75,0]),
    "car_0200": np.array([0,0,0,4.30,1.91,1.26,0]),
    "car_two_sides": np.array([0,0,0,4.30,1.91,1.26,0]),
    "one_board": np.array([0,0,0,4.30,0.05,1.26,0]),
    "three_boards": np.array([0,0,0,4.30,1.91,1.26,0]),
    "two_boards": np.array([0,0,0,4.30,1.91,1.26,0]),
    "five_boards": np.array([0,0,0,4.30,1.91,1.26,0]),
    "car_010": np.array([0,0,0,4.30,1.91,1.26,0]),
    "car_015": np.array([0,0,0,4.30,1.91,1.26,0]),
    "car_020": np.array([0,0,0,4.30,1.91,1.26,0]),
    "car_low_poly": np.array([0,0,0,4.30,1.91,1.26,0]),
    "car_side": np.array([0,0,0,4.4,2.1,1.6,0]),
    "car_side_1": np.array([0,0,0,4.4,2.2,1.6,0]),
    "car_side_2": np.array([0,0,0,4.4,2.2,1.6,0]),
    "car_side_3": np.array([0,0,0,4.4,2.2,1.6,0]),
    "car_side_4": np.array([0,0,0,4.4,2.2,1.6,0]),
    "car_side_5": np.array([0,0,0,4.4,2.2,1.6,0]),
    "car_side_6": np.array([0,0,0,4.4,2.2,1.6,0]),
    "car_side_7": np.array([0,0,0,4.4,1.6,1.6,0]),
    "car_side_8": np.array([0,0,0,4.4,1.2,1.6,0]),
    "car_side_9": np.array([0,0,0,4.4,1.0,1.6,0]),
    "adv_20_samples": np.array([0,0,0,4.4,2.16,1.62,0]),
    "adv_300_samples": np.array([0,0,0,4.39,2.18,1.65,0]),
    "adv_car_side_non_victim": np.array([0,0,0,4.42,2.19,1.67,0]),
    "adv_car_side_victim": np.array([0,0,0,4.33,2.18,1.67,0]),
    "adv_car_side_8_non_victim": np.array([0,0,0,4.45,1.28,1.64,0]),
    "two_planes_0": np.array([0,0,0,4.4,2.0,1.6,0]),
    "adv_two_planes_0_non_victim": np.array([0,0,0,4.46,2.2,1.69,0]),
    "adv_two_planes_0_victim": np.array([0,0,0,4.46,2.2,1.69,0]),
    "two_planes_10": np.array([0,0,0,4.4,2.0,1.6,0]),
    "adv_two_planes_10_non_victim": np.array([0,0,0,4.48,2.18,1.65,0]),
    "adv_two_planes_10_victim": np.array([0,0,0,4.49,2.18,1.65,0]),
    "car_10": np.array([0,0,0,4.4,2.0,1.6,0]),
    "adv_car_10_non_victim": np.array([0,0,0,4.34,2.01,1.67,0]),
    "real_car_random": np.array([0,0,0,4.4,2.2,1.6,0]),
    "adv_real_car_random_victim": np.array([0,0,0,4.4,2.2,1.6,0]),
    "real_car": np.array([0,0,0,4.4,2.2,1.6,0]),
    "adv_real_car_victim_alpha01": np.array([0,0,0,4.39,2.23,1.60,0]),
    "adv_real_car_victim_alpha1": np.array([0,0,0,4.39,2.25,1.62,0]),
    "adv_real_car_victim_alpha10": np.array([0,0,0,4.41,2.27,1.60,0]),
    "adv_real_car_victim_alpha20": np.array([0,0,0,4.40,2.21,1.62,0]),
    "adv_real_car_victim_alpha30": np.array([0,0,0,4.43,2.22,1.63,0]),
    "adv_real_car_non_victim": np.array([0,0,0,4.42,2.27,1.67,0]),
    "cuboid": np.array([0,0,0,4.4,2.2,1.6,0]),
}

class_id_map = {
    0: "unlabeled",
    1: "car",
    2: "truck",
    3: "person",
    4: "bicycle",
    5: "other"
}

class_id_inv_map = {
    "unlabeled": 0,
    "car": 1,
    "truck": 2,
    "person": 3,
    "bicycle": 4,
    "other": 5
}

color_map = {
    0: [255, 255, 255],
    1: [255, 0, 0],
    2: [255, 200, 200],
    3: [0, 255, 0],
    4: [0, 0, 255],
    5: [0, 0, 0]
}

scenario_maps = {
'2021_08_20_21_48_35': 'Town06',
'2021_08_18_19_48_05': 'Town06',
'2021_08_20_21_10_24': 'Town06',
'2021_08_21_09_28_12': 'Town06',
'2021_08_22_07_52_02': 'Town05',
'2021_08_22_09_08_29': 'Town05',
'2021_08_22_21_41_24': 'Town05',
'2021_08_23_12_58_19': 'Town05',
'2021_08_23_15_19_19': 'Town04',
'2021_08_23_16_06_26': 'Town04',
'2021_08_23_17_22_47': 'Town04',
'2021_08_23_21_07_10': 'Town10HD',
'2021_08_23_21_47_19': 'Town10HD',
'2021_08_24_07_45_41': 'Town10HD',
'2021_08_24_11_37_54': 'Town07',
'2021_08_24_20_09_18': 'Town04',
'2021_08_24_20_49_54': 'Town04',
'2021_08_24_21_29_28': 'Town04',
'2021_08_16_22_26_54': 'Town06',
'2021_08_18_09_02_56': 'Town06',
'2021_08_18_18_33_56': 'Town06',
'2021_08_18_21_38_28': 'Town06',
'2021_08_18_22_16_12': 'Town06',
'2021_08_18_23_23_19': 'Town06',
'2021_08_19_15_07_39': 'Town06',
'2021_08_20_16_20_46': 'Town06',
'2021_08_20_20_39_00': 'Town06',
'2021_08_20_21_00_19': 'Town06',
'2021_08_21_09_09_41': 'Town06',
'2021_08_21_15_41_04': 'Town05',
'2021_08_21_16_08_42': 'Town05',
'2021_08_21_17_00_32': 'Town05',
'2021_08_21_21_35_56': 'Town05',
'2021_08_21_22_21_37': 'Town05',
'2021_08_22_06_43_37': 'Town05',
'2021_08_22_07_24_12': 'Town05',
'2021_08_22_08_39_02': 'Town05',
'2021_08_22_09_43_53': 'Town05',
'2021_08_22_10_10_40': 'Town05',
'2021_08_22_10_46_58': 'Town06',
'2021_08_22_11_29_38': 'Town06',
'2021_08_22_22_30_58': 'Town05',
'2021_08_23_10_47_16': 'Town04',
'2021_08_23_11_06_41': 'Town05',
'2021_08_23_11_22_46': 'Town04',
'2021_08_23_12_13_48': 'Town05',
'2021_08_23_13_10_47': 'Town05',
'2021_08_23_16_42_39': 'Town04',
'2021_08_23_17_07_55': 'Town04',
'2021_08_23_19_27_57': 'Town10HD',
'2021_08_23_20_47_11': 'Town10HD',
'2021_08_23_22_31_01': 'Town10HD',
'2021_08_23_23_08_17': 'Town10HD',
'2021_08_24_09_25_42': 'Town07',
'2021_08_24_09_58_32': 'Town07',
'2021_08_24_12_19_30': 'Town07',
'2021_09_09_13_20_58': 'Town03',
'2021_09_09_19_27_35': 'Town01',
'2021_09_10_12_07_11': 'Town04',
'2021_09_09_23_21_21': 'Town03',
'2021_08_21_17_30_41': 'Town05',
'2021_08_22_13_37_16': 'Town06',
'2021_08_22_22_01_17': 'Town05',
'2021_08_23_10_51_24': 'Town05',
'2021_08_23_13_17_21': 'Town05',
'2021_08_23_19_42_07': 'Town10HD',
'2021_09_09_22_21_11': 'Town02',
'2021_09_11_00_33_16': 'Town10HD',
'2021_08_18_19_11_02': 'Town06'
}
